...
export function SupportWizard({ campaignId }: { campaignId: number }) {
  ...
  function canAdvance(): boolean {
    if (cfg.type === "single" && "key" in cfg) {
      return data[cfg.key as keyof WizardData] !== null;
    }
    if (cfg.type === "level") return data.level !== null;
    // multi and confirm always ok
    return true;
  }
  ...
  function next() {
    ...
    if (step === total - 1) {
      ...
      const cityCode = (data.city || "BA").slice(0, 2).toUpperCase();
      const token = `DP-${campaign.img}-${cityCode}-${num}`;
    ...
    <div>
      ...
      <div style={{ marginBottom: 8 }}>
        {cfg.type === "single" && "options" in cfg && (
          <div>
            {cfg.options!.map((opt) => {
              const isSelected = data[cfg.key as keyof WizardData] === opt;
              return (
                <button
                  key={opt}
                  ...
                  style={{
                    width: "100%",
                    textAlign: "left",
                    background: isSelected ? "rgba(124,58,237,0.1)" : "var(--surface2)",
                    border: `1.5px solid ${isSelected ? "var(--violet2)" : "var(--border)"}`,
                    color: isSelected ? "var(--violet3)" : "var(--txt)",
                    padding: "13px 16px",
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      height: 20,
                      borderRadius: "50%",
                      border: `2px solid ${isSelected ? "var(--violet)" : "var(--border3)"}`,
        ...
        {cfg.type === "multi" && "options" in cfg && (
          <div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
              {cfg.options!.map((opt) => {
                const isSelected = data.benefits.includes(opt);
                return (
                  <button
                    key={opt}
                    ...
                    style={{
                      padding: "10px 16px",
                      borderRadius: "var(--r12)",
                      border: `1.5px solid ${isSelected ? "var(--blue2)" : "var(--border)"}`,
                      background: isSelected ? "rgba(37,99,235,0.12)" : "var(--surface2)",
                      color: isSelected ? "var(--blue3)" : "var(--txt2)",
                      fontSize: 14,
                      fontFamily: "Outfit, sans-serif",
                      fontWeight: 500,
                      cursor: "pointer",
            ...
            <p style={{ fontSize: 12, color: "var(--txt3)" }}>
              Opcional — podés seleccionar varios · {data.benefits.length} seleccionado
        ...
        {cfg.type === "level" && (
          <div className="grid-3">
            {levels.map((lvl) => {
              const isSelected = data.level === lvl.name;
              return (
                <div
                  key={lvl.name}
                  onClick={() => setData((d) => ({ ...d, level: lvl.name }))}
                  style={{
                    background: isSelected ? "rgba(124,58,237,0.08)" : "var(--surface2)",
                    border: `1.5px solid ${isSelected ? "var(--violet2)" : "var(--border)"}`,
                    borderTopWidth: 3,
                    borderTopColor: lvl.color,
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 17, fontWeight: 800, color: lvl.color, marginBottom: 3 }}
                  >
                    {lvl.name}
                  </div>
        ...
        {cfg.type === "confirm" && (
          <div className="card">
            ...
            >
              {[
                ...
                >
                  ...
                  <div style={{ fontSize: 14, fontWeight: 700, color: "var(--txt)" }}>{v}</div>
                </div>
              ))}
            </div>
            ...
            {/* Explicit disclaimer on confirm step */}
            <div
              style={{
            ...
            >
              <span style={{ color: "var(--amber2)", fontWeight: 700 }}>Leé antes de confirmar: </span>
              Estás registrando una <strong style={{ color: "var(--txt)" }}>reserva condicional simulada</strong>, no comprando una entrada.
              Si el show se confirma, recibís acceso prioritario a la compra oficial en la ticketera.
              Si no se confirma, el monto nominal es devuelto íntegramente.
              DemandPass no vende tickets.
            </div>
          </div>
        )}
      </div>
      ...
      >
        {step > 0 ? (
          <button onClick={back} className="btn btn-ghost" disabled={confirming}>
            ← Atrás
          </button>
        ) : (
          <div />
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button
            ...
            className={`btn ${step === total - 1 ? "btn-success" : "btn-violet"}`}
            style={{
              opacity: isBlocked || confirming ? 0.45 : 1,
              cursor: isBlocked || confirming ? "not-allowed" : "pointer",
            }}
          >
            {confirming
              ? "Generando token..."
